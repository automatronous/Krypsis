...
(() => {
  ...
  async function render() {
    ...
    if (state) {
      state.textContent = policy.decision === "ALLOW" ? "PROTECTED" : policy.decision;
      state.style.color = policy.decision === "BLOCK" ? "#ff8590" : policy.decision === "CONFIRM" ? "#ffd37a" : "#78d6b0";
    }
    ...
    document.querySelector("#decision").innerHTML = `<div class="decision">${safe(policy.decision)}</div><div class="reason">${safe(policy.reasons.join(" "))}</div>`;
    document.querySelector("#evidence").innerHTML = policy.evidence.length ? policy.evidence.map(
      (item) => `<div><b>${safe(item.rule)}</b> \u2014 ${safe(item.detail)}</div>`
    ).join("") : "No restriction evidence.";
